...
class ScreenshotRepository(private val context: Context) {
    ...
    suspend fun importShared(uri: Uri, sourceApp: String?): Long? {
        ...
        if (id != null && id > 0) {
            ...
            if (settings.runOcrAutomatically) OcrQueueWorker.start(context)
        }
        return id
    }
    ...
    fun enqueueOcr(id: Long, force: Boolean = false) {
        val req = OneTimeWorkRequestBuilder<OcrWorker>()
            .setInputData(workDataOf(OcrWorker.KEY_ID to id, OcrWorker.KEY_FORCE to force))
            .build()
    ...
    suspend fun enqueuePendingOcr() = withContext(Dispatchers.IO) {
        if (dao.pendingOcr(1).isNotEmpty()) OcrQueueWorker.start(context)
    }
    ...
    suspend fun deleteOriginalLegacy(id: Long) = withContext(Dispatchers.IO) {
        ...
        if (item.sourceKind == "MEDIASTORE") {
            runCatching { context.contentResolver.delete(Uri.parse(item.contentUri), null, null) }
    ...
    suspend fun search(query: String, snapshot: List<ScreenshotEntity>, category: String? = null, limit: Int = 80): List<ScreenshotEntity> = withContext(Dispatchers.Default) {
        ...
        val index = if (cachedSnapshot === snapshot && cachedIndex != null) {
            cachedIndex!!
        } else {
            LocalSearchIndex(snapshot.map { it.toCoreRecord() }).also {
                cachedSnapshot = snapshot
                cachedIndex = it
            }
        }
        val byId = snapshot.associateBy { it.id }
        index.search(query, limit, categoryEnum).mapNotNull { hit -> byId[hit.record.id] }
    }
}
...
fun ScreenshotEntity.toCoreRecord() = ScreenshotRecord(
    id = id,
    title = title,
